...
const char *pub_mil = "/b2w/mil";
const char *pub_msg = "/b2w/msg";  // Debug only

// ----------------------------------------------------------------
// --------------     Subscribe topics     ------------------------
//
char topic_w2b[24] = "/w2b";     // single topic: payload 1-6
...
// Forward payload (1-6) to TM4C as single line; TM4C Parser uses payload as command.
  ...
  while (Serial.available() > 0)  // Wait for date from the TM4C
  {
    ...
    if (inchar != '\n') {        // if the incoming character is not a newline then process byte the buffer position
      ...
      if (bufpos > 0) {
        ...
        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_mode);
        client.publish(topic_publish, clk_mode, false);  // retain=0 so web doesn't get stale overwrites

        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_hour);
        client.publish(topic_publish, hour, false);

        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_min);
        client.publish(topic_publish, minute, false);

        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_sec);
        client.publish(topic_publish, second, false);

        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_mil);
        client.publish(topic_publish, mil[0] ? mil : "0", false);
      }
